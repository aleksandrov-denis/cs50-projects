...
bool lookup(string s)
{
    ...
    {
        mid = (low + high) / 2;
        //compare letters in string s and words in DICTIONARY
        if (strcmp(s, dictionary.words[mid].letters) == 0)
        {
            //if word has already been found and flagged, return false
            if (dictionary.words[mid].found == true)
            {
                printf("error 1\n");
                return false;
            }
            //if word has been found, flag it and return true
            else
            {
        ...
        //search below the middle of the list
        else if (strcmp(s, dictionary.words[mid].letters) > 0)
        {
            low = mid + 1;
        }
        //search above the middle of the list
        else if (strcmp(s, dictionary.words[mid].letters) < 0)
        {
            high = mid - 1;
        }
...
void scramble(void)
{
    //points at which spot in array to 'drop' the rotated letter
    char swap[DIMENSION][DIMENSION];
    for (int x = 0; x < DIMENSION; x++)
    {
        for (int y = 0; y < DIMENSION; y++)
        {
            //'drops' letter into a 90 degree shifted spot in array
            swap[y][(DIMENSION - 1) - x] = grid[x][y];
        }
    }
    for (int x = 0; x < DIMENSION; x++)
    {
