...
struct
{
    ...
    //fixate the boards numbers, a reference board
    bool steady[9][9];
} g;
...
int main(int argc, char *argv[])
{
    ...
    do
    {
        ...
        switch (ch)
        {
            ...
            case KEY_UP:
            {
                //decreases y coordinate by one
                g.y--;
                //shows cursor one slot above the original
                show_cursor();
                //if y coordinate is on the top border, reasign it to the bottom slot of same column
                if (g.y == -1)
                {
                    g.y = 8;
                    show_cursor();
            ...
            case KEY_LEFT:
            {
                //decreases x coordinate by one
                g.x--;
                //shows cursor one slot left of the original
                show_cursor();
                //if x coordinate is on the left border, reasign it to the right most slot of same row
                if (g.x == -1)
                {
                    g.x = 8;
                    show_cursor();
            ...
            case KEY_DOWN:
            {
                //increases y coordinate by one
                g.y++;
                //shows cursor one slot down of the original
                show_cursor();
                //if y coordinate is on the bottom border, reasign it to the top slot of same column
                if (g.y == 9)
                {
                    g.y = 0;
                    show_cursor();
            ...
            case KEY_RIGHT:
            {
                //increases x coordinate by one
                g.x++;
                //shows cursor one slot right of the original
                show_cursor();
                //if x coordinate is on the right border, reasign it to the left most slot of same row
                if (g.x == 9)
                {
                    g.x = 0;
                    show_cursor();
            ...
            //in the case that ch = 1 through 9
            case '1'...'9':
            {
                //when the change isn't directed towards OG #, proceed
                if (g.steady[g.y][g.x] != true)
                {
                    //ASCII
                    ch = ch - 48;
                    //assign to slot on boards
                    g.board[g.y][g.x] = ch;
                    //update board
                    draw_numbers();
                ...
                //when change is directed towards OG numbers, stop
                else if (g.steady[g.y][g.x] == true)
                {
                    // shut down ncurses
                    shutdown();
                    // invalid
                    printf("\nINVALID MOVE! CANNOT CHANGE OG #s!\n\n");
                    return 0;
                }
            }
            case '0': case KEY_BACKSPACE: case KEY_DC: case '.':
            {
                //when erase isn't directed towards OG #, continue
                if (g.steady[g.y][g.x] != true)
                {
                    //assign 0 a.k.a. '.' to desired position on board
                    g.board[g.y][g.x] = 0;
                    //update board
                    draw_numbers();
                    //update cursor
                ...
                else if (g.steady[g.y][g.x] == true)
                {
                    ...
                    shutdown();
                    // invalid
                    printf("\nINVALID MOVE! CANNOT REMOVE OG #s!\n\n");
                    return 0;
                }
            }
...
bool load_board(void)
{
    ...
    //assigns same values from g.board to reference board for fixated numbers
    for (int s = 0; s < 9; s++)
    {
        for (int m = 0; m < 9; m++)
        {
            g.steady[s][m] = g.board[s][m];
            //marks reference board as true if position holds an OG number
            if (g.steady[s][m] != 0)
            {
                g.steady[s][m] = true;
            }
            //marks reference board as false if position held a 0 originally
            if (g.steady[s][m] == 0)
            {
                g.steady[s][m] = false;
            }
